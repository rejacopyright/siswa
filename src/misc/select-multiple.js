  ...
  render() {
    return (
      <div className={this.props.className}>
        ...
        <select defaultValue={this.props.selected.map(x => x.id)} name={this.props.name} style={{width:100+'%'}} disabled={this.props.disabled} ref={i => this.select = i} multiple >
          {
            this.props.selected.map((r, index) => (
              <option key={index} value={r.id}>{r.text}</option>
            ))
          }
        </select>
      </div>
